...
function updateUserInfoUI(user) {
    ...
    $('#home-score-mini').text(Number(user.totalScore || 0).toLocaleString());
...
//  SOCIAL FEED
// ===============================================================
let _socialFeedItems = [];
let _socialFeedPage = 0;
const SOCIAL_FEED_PAGE_SIZE = 5;
...
function renderSocialFeedPage() {
    const container = $('#home-social-feed');
    const totalItems = _socialFeedItems.length;
    const totalPages = Math.max(1, Math.ceil(totalItems / SOCIAL_FEED_PAGE_SIZE));
    _socialFeedPage = Math.min(Math.max(_socialFeedPage, 0), totalPages - 1);

    const start = _socialFeedPage * SOCIAL_FEED_PAGE_SIZE;
    const pageItems = _socialFeedItems.slice(start, start + SOCIAL_FEED_PAGE_SIZE);
    const feedHtml = pageItems.map(item => {
    ...
    const controlsHtml = totalItems > SOCIAL_FEED_PAGE_SIZE
        ? `<div class="feed-pager">
            <button class="feed-pager-btn" onclick="changeSocialFeedPage(-1)" ${_socialFeedPage === 0 ? 'disabled' : ''}>
                <i class="fas fa-chevron-left"></i><span>ก่อนหน้า</span>
            </button>
            <span class="feed-pager-status">${start + 1}-${Math.min(start + SOCIAL_FEED_PAGE_SIZE, totalItems)} จาก ${totalItems}</span>
            <button class="feed-pager-btn" onclick="changeSocialFeedPage(1)" ${_socialFeedPage >= totalPages - 1 ? 'disabled' : ''}>
                <span>ถัดไป</span><i class="fas fa-chevron-right"></i>
            </button>
        </div>`
        : '';

    container.html(feedHtml + controlsHtml);
}

function changeSocialFeedPage(delta) {
    _socialFeedPage += delta;
    renderSocialFeedPage();
}
...
async function loadHomeTodayTasks(activities) {
    const container = $('#home-today-tasks');
    if (!container.length) return;

    const tasks = [];
    const firstPendingActivity = (activities || AppState._lastActivities || []).find(a => !a.userHasSubmitted);

    try {
        const quizStatus = await callApi('/api/game/daily-question', { lineUserId: AppState.lineProfile.userId });
        $('#home-kyt-status-mini').text(quizStatus.played ? 'แล้ว' : 'ยัง');
        tasks.push({
            icon: quizStatus.played ? 'fa-check-circle' : 'fa-helmet-safety',
            tone: quizStatus.played ? 'done' : 'primary',
            title: quizStatus.played ? 'KYT วันนี้เรียบร้อย' : 'ตอบ KYT วันนี้',
            sub: quizStatus.played ? 'รักษา streak ต่อไป พรุ่งนี้ค่อยกลับมาเล่นใหม่' : 'ตอบคำถามสั้น ๆ เพื่อรักษา streak และรับเหรียญ',
            actionText: quizStatus.played ? 'เสร็จแล้ว' : 'เล่น KYT',
            action: quizStatus.played ? '' : 'startDailyQuiz()',
            done: !!quizStatus.played
        });
    } catch (_) {
        $('#home-kyt-status-mini').text('-');
        tasks.push({
            icon: 'fa-helmet-safety',
            tone: 'muted',
            title: 'KYT วันนี้',
            sub: 'เช็กสถานะไม่ได้ชั่วคราว',
            actionText: 'เปิดเกม',
            action: "$('[data-page=game-page]').trigger('click')"
        });
    }

    if (firstPendingActivity) {
        tasks.push({
            icon: 'fa-paper-plane',
            tone: 'success',
            title: 'มีกิจกรรมที่ยังไม่ได้ส่ง',
            sub: firstPendingActivity.title || 'เลือกกิจกรรมแล้วส่งรายงาน',
            actionText: 'ไปส่ง',
            action: "$('[data-page=activities-page]').trigger('click')"
        });
    } else {
        tasks.push({
            icon: 'fa-clipboard-check',
            tone: 'done',
            title: 'กิจกรรมล่าสุดครบแล้ว',
            sub: 'ยังไม่มีงานค้างจากกิจกรรมที่แสดงบนหน้าหลัก',
    ...
    try {
        ...
        if (round?.featureEnabled && !round.isClosed && !round.maintenanceMode) {
            tasks.push({
                icon: 'fa-ticket-alt',
                tone: 'lottery',
                title: 'Safety Lottery เปิดอยู่',
                sub: `งวด ${round.drawDate || round.roundId} เหลือประมาณ ${Number(round.hoursLeft || 0)} ชม.`,
                actionText: 'เข้า Lottery',
    ...
    tasks.push({
        icon: 'fa-shield-alt',
        tone: 'muted',
        title: 'พบจุดเสี่ยงหรือ Near Miss?',
        sub: 'ส่งรายงานทันทีเพื่อให้ทีมเห็นและแก้ไขเร็วขึ้น',
    ...
    container.html(`
        <div class="today-tasks-card">
            <div class="today-tasks-head">
                <div>
                    <div class="today-tasks-kicker">Today</div>
                    <div class="today-tasks-title">สิ่งที่ควรทำวันนี้</div>
                </div>
                <span class="today-tasks-count">${openCount} งาน</span>
            </div>
            <div class="today-tasks-list">
                ${visibleTasks.map(task => `
                    <button type="button" class="today-task-item ${task.done ? 'is-done' : ''}" ${task.action ? `onclick="${task.action}"` : 'disabled'}>
                        <span class="today-task-icon tone-${task.tone}"><i class="fas ${task.icon}"></i></span>
                        <span class="today-task-copy">
                            <strong>${sanitizeHTML(task.title)}</strong>
                            <small>${sanitizeHTML(task.sub)}</small>
                        </span>
                        <span class="today-task-action">${sanitizeHTML(task.actionText)}<i class="fas fa-chevron-right"></i></span>
                    </button>
                `).join('')}
            </div>
        </div>
...
async function loadHomeDashboard(activities) {
    ...
    try {
        ...
        if (!rows || !rows.length) {
            ...
            const footerHtml = rows.length > 5
                ? `<button type="button" class="home-section-more" onclick="$('[data-page=leaderboard-page]').trigger('click')">
                    ดูอันดับทั้งหมด <i class="fas fa-chevron-right"></i>
                   </button>`
                : '';
            container.html(previewRows.map((r, i) => {
    ...
    // Load Today's tasks, Lottery Pulse and Safety Pulse
    loadHomeTodayTasks(list || []);
...
let _lotterySelectedType = 'two'; // 'two' | 'three' | 'six'
let _lotteryCurrentQuestion = null;
let _lotteryLastQuizAnswerId = null;
let _lotteryCountdownInterval = null;
let _lotterySettings = {
    priceTwo: 10,
    priceThree: 30,
    prizeSix: 100000,
    priceSix: 100,
...
const LOTTERY_STATUS_TH = {
    open: 'เปิดรับตั๋ว',
    closed: 'ปิดรับตั๋ว',
    pending_manual: 'รอกรอกผล',
    pending_confirm: 'รอยืนยันผล',
    confirmed: 'ยืนยันผลแล้ว',
    completed: 'จ่ายรางวัลแล้ว'
};

function updateLotteryPurchaseSummary(todayCount = null) {
    const coins = Number(AppState.currentUser?.coinBalance || 0);
    const price = _lotterySelectedType === 'six' ? _lotterySettings.priceSix
                : _lotterySelectedType === 'two' ? _lotterySettings.priceTwo
                : _lotterySettings.priceThree;
    $('#lottery-summary-coins').text(coins.toLocaleString());
    $('#lottery-summary-price').text(price.toLocaleString());
    if (todayCount !== null) {
        $('#lottery-summary-quota').text(`${todayCount}/${_lotterySettings.dailyLimit}`);
    }
}

function applyLotterySettings(settings = {}) {
    _lotterySettings = {
        priceTwo:   Number(settings.priceTwo)   || 10,
        priceThree: Number(settings.priceThree) || 30,
        priceSix:   Number(settings.priceSix)   || 100,
        prizeTwo:   Number(settings.prizeTwo)   || 500,
        prizeThree: Number(settings.prizeThree) || 3000,
        prizeSix:   Number(settings.prizeSix)   || 100000,
        dailyLimit: Number(settings.dailyLimit) || 5
    };

    $('#type-two-card .lottery-type-price').text(`${_lotterySettings.priceTwo.toLocaleString()} เหรียญ`);
    $('#type-three-card .lottery-type-price').text(`${_lotterySettings.priceThree.toLocaleString()} เหรียญ`);
    $('#type-six-card .lottery-type-price').text(`${_lotterySettings.priceSix.toLocaleString()} เหรียญ`);
    $('#type-two-card .lottery-type-prize').text(`รางวัล ${_lotterySettings.prizeTwo.toLocaleString()} pts`);
    $('#type-three-card .lottery-type-prize').text(`รางวัล ${_lotterySettings.prizeThree.toLocaleString()} pts`);
    $('#type-six-card .lottery-type-prize').text(`รางวัล ${_lotterySettings.prizeSix.toLocaleString()} pts`);
    $('#lottery-daily-limit-text').html(`<i class="fas fa-info-circle me-1 text-primary"></i>ซื้อได้สูงสุด ${_lotterySettings.dailyLimit.toLocaleString()} ใบ/วัน`);
...
// -----------------------------------------------
function showLotteryRules(options = {}) {
    const { firstTime = false } = options;
    const priceTwo = _lotterySettings.priceTwo.toLocaleString();
    const priceThree = _lotterySettings.priceThree.toLocaleString();
    ...
    return Swal.fire({
        ...
        confirmButtonColor: '#06C755',
        confirmButtonText: firstTime ? 'รับทราบและดำเนินการต่อ' : 'ปิด',
...
async function openLotteryModal(forceRoundId = null) {
    ...
    // แสดง T&C ครั้งแรก
    if (!localStorage.getItem('lottery_tnc_v1_accepted')) {
        try {
            const preview = await callApi('/api/lottery/current-round', {
                lineUserId: AppState.lineProfile.userId
            });
            if (preview?.settings) applyLotterySettings(preview.settings);
        } catch (_) {}
        const result = await showLotteryRules({ firstTime: true });
        if (!result.isConfirmed) return;
        localStorage.setItem('lottery_tnc_v1_accepted', '1');
    }
    ...
    document.getElementById('lottery-modal').addEventListener('hidden.bs.modal', () => {
        clearInterval(_lotteryCountdownInterval);
        _lotteryCountdownInterval = null;
    }, { once: true });
...
async function loadLotteryCurrentRound(forceRoundId = null) {
    try {
        ...
        $('#lottery-daily-quota-badge').text(`${ticketsRes.todayCount}/${_lotterySettings.dailyLimit} ใบวันนี้`);
...
async function loadLotteryGoldEligibility() {
    ...
    try {
        ...
        if (res.eligible) {
            const since = res.incidentFreeSince
                ? new Date(res.incidentFreeSince).toLocaleDateString('th-TH', { month: 'short', day: 'numeric', year: 'numeric' })
                : null;
            const sinceText = since ? ` (ปลอดภัยตั้งแต่ ${since})` : '';
            $status.html(`<span class="badge bg-warning text-dark me-1"><i class="fas fa-shield-check me-1"></i>แผนกปลอดภัย ✓</span>${sanitizeHTML(res.department || '')}${sinceText}<br><small class="text-muted">รับตั๋ว 3 ตัวท้ายฟรี 1 ใบ</small>`);
            $btn.prop('disabled', false).text('รับฟรี');
            return;
        }

        $box.addClass(res.alreadyClaimed ? 'claimed' : 'locked');
        const reason = res.reason || 'ยังไม่มีสิทธิ์รับตั๋วทอง';
        const infoIcon = `<i class="fas fa-info-circle text-muted ms-1" title="Gold Ticket ฟรี = แผนกไม่มีรายงานอุบัติเหตุใน 30 วันที่ผ่านมา" data-bs-toggle="tooltip"></i>`;
        $status.html(`${sanitizeHTML(reason)}${infoIcon}`);
        $btn.prop('disabled', true).text(res.alreadyClaimed ? 'รับแล้ว' : 'ล็อก');
        // init tooltip
        const tip = $status.find('[data-bs-toggle="tooltip"]')[0];
        if (tip) new bootstrap.Tooltip(tip);
    } catch (e) {
        $box.addClass('locked');
...
async function claimLotteryGoldTicket() {
    const confirm = await Swal.fire({
        icon: 'info',
        title: '🏆 Gold Ticket ฟรี',
        html: `<p class="mb-2">ระบบจะ<strong>สุ่มเลข 3 ตัว</strong>ให้อัตโนมัติ คุณไม่สามารถเลือกเลขได้</p>
               <p class="text-muted small mb-0">ตั๋วทองใช้ได้ 1 ใบต่อรอบเท่านั้น</p>`,
        confirmButtonText: '<i class="fas fa-gift me-1"></i>รับตั๋วทองเลย',
        cancelButtonText: 'ยกเลิก',
        showCancelButton: true,
        confirmButtonColor: '#f59e0b',
        cancelButtonColor: '#6c757d'
...
async function adminSaveLotteryPrizeConfig() {
    const prizeTwo   = Number($('#cfg-prize-two').val());
    const prizeThree = Number($('#cfg-prize-three').val());
    const prizeSix   = Number($('#cfg-prize-six').val());
    const priceTwo   = Number($('#cfg-price-two').val());
    const priceThree = Number($('#cfg-price-three').val());
    const priceSix   = Number($('#cfg-price-six').val());
    const dailyLimit = Number($('#cfg-daily-limit').val());
    if (!prizeTwo || !prizeThree || !prizeSix || !priceTwo || !priceThree || !priceSix || !dailyLimit)
        return Swal.fire('ข้อมูลไม่ถูกต้อง', 'กรุณากรอกตัวเลขที่มากกว่า 0 ทุกช่อง', 'warning');
    try {
        const userEnabled = $('#admin-lottery-user-enabled').is(':checked');
        const disabledMessage = $('#admin-lottery-disabled-message').val()?.trim() || '';
        await callApi('/api/admin/lottery/settings', {
            requesterId: AppState.lineProfile.userId,
            userEnabled, disabledMessage,
            prizeTwo, prizeThree, prizeSix,
            priceTwo, priceThree, priceSix, dailyLimit
        }, 'POST');
        showToast('บันทึกการตั้งค่าราคาและรางวัลแล้ว', 'success');
...
function selectLotteryType(type) {
    ...
    $('#type-two-card, #type-three-card, #type-six-card').removeClass('active');
    $(`#type-${type}-card`).addClass('active');
    updateLotteryPurchaseSummary();

    const input = $('#lottery-number-input');
    const digits = type === 'six' ? 6 : type === 'two' ? 2 : 3;
    const example = type === 'six' ? 'เช่น 536077' : type === 'two' ? 'เช่น 47' : 'เช่น 283';
    const hint    = type === 'six' ? '(000000-999999)' : type === 'two' ? '(00-99)' : '(000-999)';
    input.attr('maxlength', digits).attr('placeholder', example);
    $('#lottery-digit-hint').text(hint);
    input.val('');
    updateLotteryCharCounter();

    input.off('input.counter').on('input.counter', () => {
        input.val(input.val().replace(/\D/g, '').slice(0, digits));
        updateLotteryCharCounter();
    });
}

function updateLotteryCharCounter() {
    const digits = _lotterySelectedType === 'six' ? 6 : _lotterySelectedType === 'two' ? 2 : 3;
    const current = String($('#lottery-number-input').val()).length;
    let $counter = $('#lottery-char-counter');
    if (!$counter.length) {
        $('#lottery-number-input').closest('.lottery-number-input-area').append(
            '<div id="lottery-char-counter" class="text-end small mt-1 text-muted"></div>'
        );
        $counter = $('#lottery-char-counter');
    }
    const color = current === digits ? 'text-success fw-semibold' : 'text-muted';
    $counter.attr('class', `text-end small mt-1 ${color}`).text(`${current}/${digits} หลัก`);
}
...
async function startLotteryQuiz() {
    ...
    const $btn = $('#btn-lottery-buy');
    $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>กำลังโหลดคำถาม...');
...
async function answerLotteryQuiz(selectedOption) {
    ...
    try {
        ...
        if (res.isCorrect) {
            ...
            // แสดงเฉลย + explanation แล้วเปิดคำถามใหม่
            const $body = $('#lottery-quiz-body');
            const explanationHtml = res.explanation
                ? `<div class="lottery-quiz-explanation mt-2">
                       <i class="fas fa-lightbulb me-1 text-warning"></i>
                       <span>${sanitizeHTML(res.explanation)}</span>
                   </div>`
                : '';
            const wrongHtml = `
                <div class="alert alert-danger mt-3 text-center mb-2">
                    <i class="fas fa-times-circle me-1"></i>ตอบผิด! เฉลย: <strong>${res.correctOption}</strong>
                </div>
                ${explanationHtml}
                <button class="btn btn-warning w-100 fw-bold mt-3" onclick="retryLotteryQuiz()">
...
// -----------------------------------------------
// cancelLotteryQuiz — ยกเลิกและกลับไปหน้าเลือกเลข
// -----------------------------------------------
function cancelLotteryQuiz() {
    _lotteryCurrentQuestion = null;
...
async function executeBuyTicket() {
    ...
    try {
        ...
        const typeTh = _lotterySelectedType === 'six' ? '⭐ รางวัลที่ 1 (6 ตัวตรง)' : _lotterySelectedType === 'two' ? '🟢 2 ตัวท้าย' : '🔴 3 ตัวท้าย';
        await Swal.fire({
            icon: 'success',
...
async function loadMyLotteryTickets() {
    ...
    try {
        const [res, statsRes] = await Promise.all([
            callApi('/api/lottery/my-tickets', { lineUserId: AppState.lineProfile.userId }),
            callApi('/api/lottery/stats', { lineUserId: AppState.lineProfile.userId }).catch(() => null)
        ]);
        const tickets = res.tickets || [];

        const userStats = statsRes?.userStats;
        const statsHtml = userStats ? `
            <div class="lottery-personal-stats mb-3">
                <div>
                    <span>${Number(userStats.myTickets || 0).toLocaleString()}</span>
                    <small>ตั๋วทั้งหมด</small>
                </div>
                <div>
                    <span class="text-warning">${Number(userStats.lotteryWinCount || 0).toLocaleString()}</span>
                    <small>ครั้งที่ถูก</small>
                </div>
                <div>
                    <span class="text-success">${Number(userStats.lotteryTotalWinnings || 0).toLocaleString()}</span>
                    <small>pts รวม</small>
                </div>
            </div>` : '';

        if (!tickets.length) {
            $list.html(statsHtml + `<div class="empty-state-small"><i class="fas fa-ticket-alt"></i><p>ยังไม่มีตั๋ว</p></div>`);
            return;
        }

        // ตรวจ winner celebration (เฉพาะงวดที่ completed + isWinner ที่ยังไม่ celebrate)
        const newWins = tickets.filter(t => t.isWinner && t.roundStatus === 'completed');
        if (newWins.length) {
            const celebKey = `lottery_win_celebrated_${newWins.map(t => t.ticketId).sort().join('_')}`;
            if (!localStorage.getItem(celebKey)) {
                localStorage.setItem(celebKey, '1');
                fireConfetti('big');
                const totalWon = newWins.reduce((s, t) => s + Number(t.prizeAmount || 0), 0);
                await Swal.fire({
                    icon: 'success',
                    title: '🎉 คุณถูกรางวัล!',
                    html: `<p>ถูกรางวัล <strong>${newWins.length} ใบ</strong><br>รวม <strong class="text-success">${totalWon.toLocaleString()} คะแนน</strong></p>`,
                    confirmButtonColor: '#06C755',
                    confirmButtonText: 'ดูตั๋วที่ถูก'
                });
            }
        }
        ...
        const recentWins = tickets.filter(t => t.isWinner && t.roundStatus === 'completed');
        const winBanner = recentWins.length
            ? `<div class="lottery-win-banner mb-3">
                <i class="fas fa-trophy me-2"></i>
                คุณถูกรางวัล <strong>${recentWins.length} ใบ</strong> รวม <strong>${recentWins.reduce((s,t) => s+Number(t.prizeAmount||0), 0).toLocaleString()} pts</strong>!
               </div>` : '';

        let html = statsHtml + winBanner;
...
async function loadAdminLotteryDashboard() {
    ...
    try {
        ...
        const statusColorMap = {
            open: 'primary', closed: 'secondary', pending_confirm: 'warning',
            pending_manual: 'danger', confirmed: 'info', completed: 'success'
        };
        let roundsHtml = '';
        (rounds || []).forEach(r => {
        ...
        $el.html(`
            ...
            <div class="card mb-3 border-0 bg-light">
                <div class="card-body py-2 px-3">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h6 class="mb-0 fw-bold small"><i class="fas fa-cog me-1 text-secondary"></i>ตั้งค่าราคาและรางวัล</h6>
                        <button class="btn btn-sm btn-success" onclick="adminSaveLotteryPrizeConfig()">
                            <i class="fas fa-save me-1"></i>บันทึก
                        </button>
                    </div>
                    <div class="row g-2">
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">ราคา 2D (เหรียญ)</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-price-two" value="${settings.priceTwo || 10}" min="1">
                        </div>
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">ราคา 3D (เหรียญ)</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-price-three" value="${settings.priceThree || 30}" min="1">
                        </div>
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">ราคา 6D (เหรียญ)</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-price-six" value="${settings.priceSix || 100}" min="1">
                        </div>
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">รางวัล 2D (pts)</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-prize-two" value="${settings.prizeTwo || 500}" min="1">
                        </div>
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">รางวัล 3D (pts)</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-prize-three" value="${settings.prizeThree || 3000}" min="1">
                        </div>
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">รางวัล 6D (pts)</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-prize-six" value="${settings.prizeSix || 100000}" min="1">
                        </div>
                        <div class="col-4">
                            <label class="form-label form-label-sm mb-1">จำกัด/วัน</label>
                            <input type="number" class="form-control form-control-sm" id="cfg-daily-limit" value="${settings.dailyLimit || 5}" min="1" max="100">
                        </div>
                    </div>
                </div>
            </div>
            ...
            ${(() => {
                const openRounds = (rounds || []).filter(r => r.status === 'open' && !r.isTest);
                if (!openRounds.length) return '';
                const options = openRounds.map(r =>
                    `<option value="${sanitizeHTML(r.roundId)}">${sanitizeHTML(r.drawDate)}</option>`
                ).join('');
                return `<div class="card border-0 bg-light p-3">
                    <div class="d-flex justify-content-between align-items-center gap-2">
                        <div>
                            <div class="fw-bold small"><i class="fas fa-bell me-1 text-success"></i>แจ้งเตือน LINE ผู้ใช้ทุกคน</div>
                            <small class="text-muted">ส่ง push แจ้งว่างวดใหม่เปิดรับตั๋วแล้ว</small>
                        </div>
                        <div class="d-flex gap-2 align-items-center">
                            <select class="form-select form-select-sm" id="admin-broadcast-round-select" style="min-width:110px;">${options}</select>
                            <button class="btn btn-success btn-sm fw-semibold text-nowrap" onclick="adminBroadcastLotteryNotification()">
    ...
    try {
        ...
        const winnerRows = winners.length ? winners.map(w => `
            <tr>
                ...
                <td><span class="badge bg-${w.ticketType === 'six' ? 'warning text-dark' : w.ticketType === 'two' ? 'success' : 'danger'}">${w.ticketType === 'six' ? '6D' : w.ticketType === 'two' ? '2D' : '3D'}</span> ${w.isGoldTicket ? '<span class="badge bg-warning text-dark">Gold</span>' : ''}</td>
        ...
        const currentOffset = Number(res.ticketOffset || 0);
        const hasMore = tickets.length >= 80;
        const ticketRows = tickets.length ? tickets.map(t => `
            <tr>
                ...
                <td>${t.ticketType === 'six' ? '6D' : t.ticketType === 'two' ? '2D' : '3D'} ${t.isGoldTicket ? '<span class="badge bg-warning text-dark">Gold</span>' : ''}</td>
        ...
        const loadMoreHtml = hasMore
            ? `<tr><td colspan="5" class="text-center py-2">
                 <button class="btn btn-outline-secondary btn-sm" onclick="loadAdminLotteryMonitorMore(${currentOffset + 80})">
                   <i class="fas fa-chevron-down me-1"></i>โหลดเพิ่มเติม
                 </button>
               </td></tr>`
            : '';
        const exportBtn = `<a class="btn btn-outline-success btn-sm" href="/api/admin/lottery/export?requesterId=${encodeURIComponent(AppState.lineProfile.userId)}${currentSelect ? '&roundId=' + encodeURIComponent(currentSelect) : ''}" target="_blank">
        ...
        $wrap.html(`
            ...
            <div class="row g-3">
                ...
                <div class="col-lg-6">
                    <div class="lottery-admin-panel">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="lottery-admin-panel-title mb-0">Recent Tickets</div>
                            ${exportBtn}
                        </div>
                        <div class="table-responsive mt-2"><table class="table table-sm align-middle mb-0" id="admin-tickets-table">
                            <thead><tr><th>User</th><th>Round</th><th>Type</th><th>No.</th><th>Status</th></tr></thead>
                            <tbody>${ticketRows}${loadMoreHtml}</tbody>
                        </table></div>
                    </div>
                </div>
                <div class="col-12">
...
async function loadAdminLotteryMonitorMore(offset) {
    const roundId = $('#admin-monitor-round-select').val();
    try {
        const params = { requesterId: AppState.lineProfile.userId, offset };
        if (roundId) params.roundId = roundId;
        const res = await callApi('/api/admin/lottery/monitor', params);
        const tickets = res.tickets || [];
        const hasMore = tickets.length >= 80;
        const currentOffset = Number(res.ticketOffset || 0);
        const newRows = tickets.map(t => `
            <tr>
                <td><strong>${sanitizeHTML(t.fullName || '-')}</strong><br><small class="text-muted">${sanitizeHTML(t.employeeId || '')}</small></td>
                <td>${sanitizeHTML(t.roundId)}</td>
                <td>${t.ticketType === 'six' ? '6D' : t.ticketType === 'two' ? '2D' : '3D'} ${t.isGoldTicket ? '<span class="badge bg-warning text-dark">Gold</span>' : ''}</td>
        ...
        const loadMoreHtml = hasMore
            ? `<tr><td colspan="5" class="text-center py-2"><button class="btn btn-outline-secondary btn-sm" onclick="loadAdminLotteryMonitorMore(${currentOffset + 80})"><i class="fas fa-chevron-down me-1"></i>โหลดเพิ่มเติม</button></td></tr>`
            : '';
        $('#admin-tickets-table tbody tr:last-child').remove();
        $('#admin-tickets-table tbody').append(newRows + loadMoreHtml);
    } catch (e) {
        showToast('โหลดเพิ่มเติมไม่สำเร็จ: ' + e.message, 'danger');
    }
}
...
async function adminSaveLotteryResult() {
    const roundId     = $('#admin-lottery-round-select').val();
    const first_prize = $('#admin-lottery-first-prize').val().trim();
    const last2       = $('#admin-lottery-last2').val().trim();
    const last3b      = $('#admin-lottery-last3b').val().trim();
    const last3b2     = $('#admin-lottery-last3b2').val().trim();
    const last3f      = $('#admin-lottery-last3f').val().trim();
    const last3f2     = $('#admin-lottery-last3f2').val().trim();

    if (!roundId || !last2 || !last3b) {
        return Swal.fire('ข้อมูลไม่ครบ', 'กรุณาเลือกงวดและกรอก 2 ตัวท้าย + 3 ตัวท้าย ชุด 1', 'warning');
    }
    try {
        await callApi('/api/admin/lottery/set-result', {
            requesterId: AppState.lineProfile.userId,
            roundId, first_prize: first_prize || null, last2,
            last3_back: last3b, last3_back2: last3b2 || null,
            last3_front: last3f || null, last3_front2: last3f2 || null
        }, 'POST');
...
async function adminFetchLotteryResultWithAI() {
    const roundId = $('#admin-lottery-round-select').val();
    if (!roundId) return Swal.fire('เลือกงวดก่อน', '', 'warning');

    const ok = await Swal.fire({
        icon: 'question',
        title: 'ให้ AI ดึงผลรางวัล?',
        html: `<p class="mb-1">งวด <strong>${sanitizeHTML(roundId)}</strong></p><p class="small text-muted mb-0">ระบบจะดึงจากแหล่งผลรางวัลและเติมเลขให้ตรวจสอบก่อนยืนยัน</p>`,
        showCancelButton: true,
        confirmButtonColor: '#06C755',
        confirmButtonText: 'ดึงผลด้วย AI',
        cancelButtonText: 'ยกเลิก'
    });
    if (!ok.isConfirmed) return;

    Swal.fire({ title: 'AI กำลังดึงผลรางวัล...', didOpen: () => Swal.showLoading(), allowOutsideClick: false });
    try {
        const res = await callApi('/api/admin/lottery/fetch-result', {
            requesterId: AppState.lineProfile.userId,
            roundId
        }, 'POST');
        $('#admin-lottery-first-prize').val(res.first_prize || '');
        $('#admin-lottery-last2').val(res.last2 || '');
        $('#admin-lottery-last3b').val(res.last3_back || '');
        $('#admin-lottery-last3b2').val(res.last3_back2 || '');
        $('#admin-lottery-last3f').val(res.last3_front || '');
        $('#admin-lottery-last3f2').val(res.last3_front2 || '');
        await Swal.fire('ดึงผลสำเร็จ', 'กรุณาตรวจสอบเลขก่อนกดยืนยันผล', 'success');
        await loadAdminLotteryDashboard();
        await loadAdminLotteryRoundSelect();
        await loadAdminLotteryMonitor(true);
...
async function adminConfirmLotteryResult() {
    ...
    Swal.fire({ title: 'กำลังตรวจรายชื่อผู้ถูกรางวัล...', didOpen: () => Swal.showLoading(), allowOutsideClick: false });
    let preview;
    try {
        preview = await callApi('/api/admin/lottery/preview-winners', {
    ...
    const winnerRows = winners.length
        ? winners.map(w => `<tr>
            <td>${sanitizeHTML(w.fullName || '-')}</td>
            <td>${sanitizeHTML(w.department || '-')}</td>
            <td>${w.ticketType === 'six' ? 'รางวัลที่ 1 (6D)' : w.ticketType === 'two' ? '2 ตัวท้าย' : '3 ตัวท้าย'}${w.isGoldTicket ? ' Gold' : ''}</td>
            <td class="fw-bold">${sanitizeHTML(w.number)}</td>
            <td class="text-success fw-bold">${Number(w.prize).toLocaleString()} pts</td>
          </tr>`).join('')
        : '<tr><td colspan="5" class="text-center text-muted">ไม่มีผู้ถูกรางวัลในงวดนี้</td></tr>';

    const confirmResult = await Swal.fire({
        icon: 'info',
        title: `ตรวจผลงวด ${sanitizeHTML(round.drawDate)}`,
        html: `
            ${testWarn}
            <p class="mb-2">ผล: ${round.first_prize ? `<strong>${sanitizeHTML(round.first_prize)}</strong> (6D) / ` : ''}<strong>${sanitizeHTML(round.last2)}</strong> (2D) / <strong>${sanitizeHTML(round.last3_back)}${round.last3_back2 ? ', ' + sanitizeHTML(round.last3_back2) : ''}</strong> (3D)</p>
            <p class="mb-2">ตั๋วทั้งหมด: <strong>${Number(totalTickets).toLocaleString()}</strong> ใบ จาก <strong>${Number(totalPlayers).toLocaleString()}</strong> คน</p>
            <p class="mb-3 text-success fw-bold">ผู้ถูกรางวัล: ${winners.length} คน | รวมรางวัล: ${Number(totalPrizesToPay).toLocaleString()} pts</p>
            <div class="table-responsive" style="max-height:200px;overflow-y:auto">
                <table class="table table-sm table-bordered mb-0">
                    <thead class="table-light"><tr><th>ชื่อ</th><th>แผนก</th><th>ประเภท</th><th>เลข</th><th>รางวัล</th></tr></thead>
                    <tbody>${winnerRows}</tbody>
                </table>
            </div>`,
        showCancelButton: true,
        confirmButtonColor: '#06C755',
        confirmButtonText: 'ยืนยันผล (ขั้นตอน 2/3)',
        cancelButtonText: 'ยกเลิก',
        width: '600px'
    });
    if (!confirmResult.isConfirmed) return;
    ...
    try {
        ...
        showToast('ยืนยันผลเรียบร้อย พร้อมประมวลผลรางวัล', 'success');
        await loadAdminLotteryDashboard();
    } catch (e) {
        Swal.fire('Error', e.message, 'error');
    }
...
async function adminProcessLotteryPrizes() {
    ...
    Swal.fire({ title: 'กำลังโหลดรายชื่อผู้ถูกรางวัล...', didOpen: () => Swal.showLoading(), allowOutsideClick: false });
    let preview;
    try {
        preview = await callApi('/api/admin/lottery/preview-winners', {
            requesterId: AppState.lineProfile.userId, roundId
    ...
    const confirm = await Swal.fire({
        ...
        html: `<p>งวด <strong>${sanitizeHTML(roundId)}</strong></p>
               ${testWarn}
               <p>ผู้ถูกรางวัล: <strong class="text-success">${winners.length} คน</strong> | รวมรางวัล: <strong class="text-success">${Number(totalPrizesToPay).toLocaleString()} pts</strong></p>
               <p class="text-danger small">การดำเนินการนี้ไม่สามารถย้อนกลับได้</p>
               <p class="small">ระบบจะจ่าย Points และส่ง LINE ให้ผู้ถูกรางวัลทุกคน</p>`,
        showCancelButton: true,
        confirmButtonColor: '#d33', confirmButtonText: 'ยืนยัน จ่ายรางวัล (ขั้นตอน 3/3)',
        cancelButtonText: 'ยกเลิก'
    });
    if (!confirm.isConfirmed) return;
...
async function adminCreateLotteryRound() {
    ...
    const isTest = $('#admin-new-round-test').is(':checked');
    if (!date) return Swal.fire('ระบุวันที่', '', 'warning');
    try {
        await callApi('/api/admin/lottery/rounds', {
            requesterId: AppState.lineProfile.userId, drawDate: date, isTest
        }, 'POST');
        showToast(`สร้างงวด ${date}${isTest ? ' (ทดสอบ)' : ''} แล้ว`, 'success');
        $('#admin-new-round-date').val('');
        $('#admin-new-round-test').prop('checked', false);
        await loadAdminLotteryDashboard();
        await loadAdminLotteryRoundSelect();
        await loadAdminLotteryMonitor(true);
    } catch (e) {
...
async function adminBroadcastLotteryNotification() {
    const roundId = $('#admin-broadcast-round-select').val();
    if (!roundId) return Swal.fire('เลือกงวดก่อน', '', 'warning');

    const confirm = await Swal.fire({
        icon: 'question',
        title: 'ส่ง LINE Push ถึงผู้ใช้ทุกคน?',
        html: `แจ้งเตือนว่างวด <strong>${sanitizeHTML(roundId)}</strong> เปิดรับตั๋วแล้ว<br><small class="text-muted">ส่งถึงผู้ใช้ทุกคนในระบบ</small>`,
        showCancelButton: true,
    ...
    try {
        ...
        const res = await callApi('/api/admin/lottery/broadcast-new-round', {
            requesterId: AppState.lineProfile.userId, roundId
        }, 'POST');
        Swal.fire({
            icon: 'success',
            title: 'ส่งเสร็จแล้ว',
            html: `ส่งสำเร็จ <strong>${res.sent}</strong> คน, ล้มเหลว <strong>${res.failed}</strong> คน`,
            confirmButtonColor: '#06C755'
        });
    } catch (e) {
        Swal.fire('Error', e.message, 'error');
...
async function adminAutoCreateLotteryRounds() {
    let previewDates = [];
    try {
        const prev = await callApi('/api/admin/lottery/preview-auto-rounds', {
            requesterId: AppState.lineProfile.userId, count: 4
        });
        previewDates = prev.preview || [];
    } catch (_) {}

    const previewHtml = previewDates.length
        ? `<ul class="text-start ps-3 mt-2">
            ${previewDates.map(p => `<li>${sanitizeHTML(p.drawDate)} ${p.exists ? '<span class="badge bg-secondary">มีอยู่แล้ว</span>' : '<span class="badge bg-success">สร้างใหม่</span>'}</li>`).join('')}
           </ul>`
        : '<p class="text-muted small">ไม่สามารถโหลด preview ได้</p>';

    const confirm = await Swal.fire({
        icon: 'question',
        title: 'สร้างงวดถัดไปอัตโนมัติ?',
        html: `<p class="mb-2">งวดที่จะสร้าง (วันที่ 1 และ 16):</p>${previewHtml}`,
        showCancelButton: true,
        confirmButtonColor: '#06C755',
        ...
        questions.forEach(q => {
            ...
            html += `<div class="card mb-2 border-0 shadow-sm">
                <div class="card-body py-2 px-3">
                    ...
                    ${q.explanation ? `<div class="d-flex align-items-start gap-1 mb-1">
                        <i class="fas fa-lightbulb text-warning mt-1" style="font-size:0.7rem;flex-shrink:0;"></i>
                        <small class="text-muted fst-italic">${sanitizeHTML(q.explanation)}</small>
                    </div>` : ''}
...
function openAddLotteryQuestionForm() {
    ...
    $('#lottery-q-text, #lottery-q-a, #lottery-q-b, #lottery-q-c, #lottery-q-d, #lottery-q-explanation').val('');
...
async function editLotteryQuestion(questionId) {
    try {
        ...
        $('#lottery-q-explanation').val(q.explanation || '');
        $('#add-lottery-q-form').show();
...
async function saveLotteryQuestion() {
    ...
    const payload = {
        ...
        explanation: $('#lottery-q-explanation').val().trim() || null,
        isActive: true
    };
